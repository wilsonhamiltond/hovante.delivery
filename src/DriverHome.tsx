...
> = {
  ...
  fr: {
    ...
    deliverTitle: 'Livrer',
    currentToOffice: (name) => `Livraison en cours · récupérez chez ${name}`,
    currentToClient: (name) => `Livraison en cours · apportez la commande à ${name}`,
    merchantFallback: 'le commerce',
    clientFallback: 'le client',
    poolAvailable: (count, filtered, radiusKm) =>
      `${count} livraison(s) disponible(s)${filtered ? ` à moins de ${radiusKm} km` : ''} · touchez un repère`,
    pendingSync: (n) => `${n} action(s) en attente de synchronisation.`,
    startRide: '🚚 Livraison en route',
    emptyChip: 'Recherche de commandes près de vous · la liste se met à jour toute seule',
    groupTitle: (n, name) => `${n} commandes chez ${name}`,
    thisSpot: 'ce point',
    groupSub: 'Choisissez celle que vous voulez voir',
    delivery: 'Livraison',
    manyOrdersTitle: (n, name) => `${n} commandes · ${name}`,
    noAddress: 'Sans adresse',
    pickupPrefix: '🏪 Récupérer : ',
    deliverPrefix: '📍 Livrer : ',
    clientPrefix: '👤 Client : ',
    client: 'Client',
    product: 'Produit',
    moreItems: (n) => `… et ${n} produit(s) de plus`,
    toCollect: 'à encaisser',
    includesShipping: (fee) => ` · livraison ${fee} incluse`,
    see: 'Voir',
  },
};
