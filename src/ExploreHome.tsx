...
> = {
  ...
  fr: {
    ...
    cartFromMerchant: (name) => `Votre commande vient de ${name}`,
    oneMerchantOnly: 'Vous ne pouvez commander que dans un seul commerce à la fois',
    emptyCart: 'Vider',
    allMerchants: '‹ Tous les commerces',
    viewItem: (name) => `Voir ${name}`,
    addItem: (name) => `Ajouter ${name}`,
    emptySearch: 'Aucun produit trouvé pour votre recherche.',
    noMerchantAt: (place) => `Aucun commerce ne livre encore à ${place}. Essayez une autre adresse.`,
    thisLocation: 'cet endroit',
    emptyCatalog: 'Aucun produit disponible pour le moment.',
    switchMerchantTitle: 'Changer de commerce',
    switchMerchantBody: (from, to) => `Votre panier contient des produits de ${from}. Le vider et ajouter de ${to} ?`,
    cancel: 'Annuler',
    emptyAndAdd: 'Vider et ajouter',
    addToCartAt: (price) => `Ajouter au panier · ${price}`,
